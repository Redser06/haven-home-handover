export function Sidebar({ token, activeTab, setActiveTab, setIsChatOpen }: { token: string, activeTab: string, setActiveTab: (v: string) => void, setIsChatOpen: (v: boolean) => void }) {
  return (
    <aside className="w-64 bg-[#1E293B] text-slate-300 flex flex-col p-4 space-y-2 border-r border-slate-800 hidden md:flex">
      ...
      <div className="mt-auto pt-4 border-t border-slate-800">
        <div className="bg-slate-800/60 rounded-xl p-3 text-xs space-y-1">
          ...
          <span className="font-mono text-emerald-400 font-semibold block select-all">{token}</span>
